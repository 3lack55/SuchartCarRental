...
export default function ArchivedDriversPage() {
    ...
    const { data, isLoading, error } = useDrivers({ search: debouncedSearch, includeInactive: true });
    const drivers = (data?.data ?? []).filter((d) => d.deleted);
    ...
    function handleRestored() {
        setSuccessMessage('กู้คืนข้อมูลคนขับเรียบร้อย');
        setSelectedDriverId(null);
    }
        ...
        <div className="space-y-5 mx-auto max-w-7xl" style={{ color: 'var(--page-text)' }}>
            <header className="flex flex-col gap-4 rounded-lg border p-5 shadow-sm md:flex-row md:items-center md:justify-between" style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--surface-border)' }}>
                <div>
                    ...
                    <h1 className="mt-1 text-2xl font-semibold" style={{ color: 'var(--page-text)' }}>คนขับที่พ้นสภาพ</h1>
                </div>
                                ...
                                >
                                    <td className="px-4 py-3" style={{ color: 'var(--page-text)' }}>
                                        <div className="flex items-center gap-3">
                                            <div className="flex h-9 w-9 items-center justify-center rounded-lg text-xs font-semibold" style={{ backgroundColor: 'var(--surface-soft)', color: 'var(--page-text)', opacity: 0.75 }}>
                                                {initials(d.first_name, d.last_name)}
                                            </div>
                                            <div>
                                                <div className="font-semibold" style={{ color: 'var(--page-text)' }}>
